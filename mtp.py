...
#     def __init__(self, backbone: nn.Module, num_modes: int, seconds: float = 6, frequency_in_hz: float = 2,
...
#         print(f"frequency_in_hz: {frequency_in_hz}")
...
#             output_size = num_modes * predictions_per_mode + num_modes
#         except Exception as e:
#             raise ValueError(f"Error calculating predictions_per_mode and output_size: {e}")
        
#         # Debugging prints to ensure calculations are correct
#         print(f"Backbone feature dim: {backbone_feature_dim}")
#         print(f"ASV_DIM: {ASV_DIM}")
#         print(f"n_hidden_layers: {n_hidden_layers}")
#         print(f"predictions_per_mode: {predictions_per_mode}")
#         print(f"Output size for fc2: {output_size}")
        
#         # Ensure output size is within a reasonable range to avoid overflow
#         if not isinstance(output_size, int) or output_size <= 0 or output_size > 10000:  # Arbitrary upper limit for sanity check
#             raise ValueError(f"Invalid output size calculated: {output_size}")
        
#         self.fc2 = nn.Linear(n_hidden_layers, output_size)
...
#         agent_state_vector = agent_state_vector.unsqueeze(1).repeat(1, backbone_features.size(1) // agent_state_vector.size(1))
...
#         self.num_modes = num_modes
#         self.num_location_coordinates_predicted = 2 #prediction of x and y coordinates at each time step
#         self.regression_loss_weight = regression_loss_weight
#         self.angle_threshold = angle_threshold_degrees
        
#     def get_trajectory_and_modes(self,model_prediction: torch.Tensor) -> Tuple[torch.Tensor,torch.Tensor]:
#         mode_probabilities = model_prediction[:, -self.num_modes, -1, self.num_location_coordinates_predicted]
...
#     @staticmethod
#     def angle_between(ref_traj:torch.Tensor, traj_to_compare: torch.Tensor)-> float:
#         EPSILON = 1e-5
#         if (ref_traj.ndim !=2 or traj_to_compare.shape[1] != 2):
#             raise ValueError("Not of the same size.")
...
#     @staticmethod
#     def compare_ave_12_norms(tensor:torch.Tensor)->float:
#         twelve_norms = torch.norm(tensor,p=2,dim=2)
#         avg_distance = torch.mean(twelve_norms)
#         return avg_distance.item()
    
#     def compute_angles_from_ground_truth(self, target: torch.Tensor,
#                                          trajectories: torch.Tensor) -> List[Tuple[float, int]]:
#         angles_from_ground_truth = []
#         for mode, mode_trajectory in enumerate(trajectories):
#             angle = self.angle_between(target[0], mode_trajectory)
#             angles_from_ground_truth.append((angle,mode))
#         return angles_from_ground_truth
    
#     def compute_best_mode(self, angles_from_ground_truth: List[Tuple[float,int]],
#                           target: torch.Tensor, trajectories: torch.Tensor) -> int:
#         angles_from_ground_truth = sorted(angles_from_ground_truth)
#         max_angle_below_threshold_index = -1
#         for angle_idx, (angle, mode) in enumerate(angles_from_ground_truth):
#             if angle <= self.angle_threshold:
#                 max_angle_below_threshold_index = angle_idx
#             else:
#                 break
#         if max_angle_below_threshold_index == -1:
#             best_mode = random.randint(0, self.num_modes-1)
#         else:
#             dist_from_ground_truth = []
#             for angle, mode in angles_from_ground_truth[: max_angle_below_threshold_index+1]:
#                 norm = self.compare_ave_12_norms(target-trajectories[mode,:,:])
                
#                 dist_from_ground_truth.append((norm,mode))
                
#             dist_from_ground_truth = sorted(dist_from_ground_truth)
#             best_mode = dist_from_ground_truth[0][1]
#         return best_mode
    
#     def __call__(self, predictions:torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
#         batch_losses = torch.Tensor().requires_grad_(True).to(predictions.device)
#         trajectories, modes = self.get_trajectory_and_modes(predictions)
#         for batch_idx in range(predictions.shape[0]):
#             angles = self.compute_angles_from_ground_truth(target=targets[batch_idx],
#                                                            trajectories=trajectories[batch_idx])
#             best_mode = self.compute_best_mode(angles, target=targets[batch_idx],
#                                                trajectories=trajectories[batch_idx])
...
# #     model = multiple_trajectory_model(backbone, num_modes)

# #     # Create random tensors
# #     image_tensor = torch.randn(batch_size, *input_shape)
# #     agent_state_vector = torch.randn(batch_size, asv_dim)

# #     # Perform forward pass
# #     try:
# #         output = model(image_tensor, agent_state_vector)
# #         print("Sanity check passed. Output shape:", output.shape)
# #     except Exception as e:
