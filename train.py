...
def collate_fn(batch):
    images, states, gts = zip(*batch)
    
    images = torch.stack(images)
    states = torch.stack(states)
    
    # Filter out empty or misformatted tensors
    valid_gts = [gt for gt in gts if gt.dim() == 2 and gt.size(1) == 2]
    
    if len(valid_gts) == 0:
        raise ValueError('All ground truth sequences are empty or misformatted.')
    
    # Pad the gts to ensure consistent batch size with images and states
    max_len = max(gt.size(0) for gt in valid_gts)
    gts_padded = [torch.cat([gt, torch.zeros(max_len - gt.size(0), gt.size(1))]) for gt in valid_gts]
    gts_padded = torch.stack(gts_padded)
    
    # Adjust images and states to match the number of valid ground truths
    images = images[:len(valid_gts)]
    states = states[:len(valid_gts)]
    
    return images, states, gts_padded
...
def main():
    ...
    train_loader = DataLoader(trainset, shuffle=True, batch_size=args.batch_size, num_workers=args.num_workers, collate_fn=collate_fn)
    val_loader = DataLoader(valset, shuffle=True, batch_size=args.batch_size, num_workers=args.num_workers, collate_fn=collate_fn)

    if args.backbone.startswith('resnet'):
        backbone = ResNetBackbone(args.backbone)
    else:
        backbone = MobileNetBackbone(args.backbone)
